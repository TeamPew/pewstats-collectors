...
def process_match(worker: TelemetryProcessingWorker, match_data: dict) -> bool:
    ...
    logger.info(
        f"Processing match {match_id} ({match_data['map_name']}, {match_data['game_mode']})"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill finishing metrics for historical matches"
    )
    parser.add_argument(
        "--limit", type=int, default=100, help="Number of matches to process (default: 100)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Process in batches (default: 10)"
    )
